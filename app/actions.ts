...
import type { ExportBoardData, ExportElement, ExportList, ExportCard } from '@/lib/exportBoard'

async function fetchExportBoard(
  ...
  const { data } = await supabase
    .from('boards')
    .select('id, name, mode, content, board_elements(*), lists(id, name, position, cards(id, title, description, position, done, deadline))')
    .eq('id', boardId)
    .eq('user_id', userId)
    .single()
  if (!data) return null
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elements: ExportElement[] = ((data.board_elements ?? []) as any[]).map((e: any) => ({
    type: String(e.type ?? ''),
    data: (e.data ?? {}) as Record<string, unknown>,
  }))

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const lists: ExportList[] = ((data.lists ?? []) as any[]).map((l: any) => ({
    name: String(l.name ?? ''),
  ...
  if (depth < 4) {
    const { data: subs } = await supabase
      ...
      .order('tab_position', { ascending: true })
    if (subs?.length) {
      const settled = await Promise.all(subs.map(s => fetchExportBoard(supabase, s.id, userId, depth + 1)))
      children = settled.filter(Boolean) as ExportBoardData[]
    }
  }
  ...
  return {
    ...
    mode: (data.mode as 'classic' | 'trello' | 'text' | 'folder') ?? 'classic',
...
export async function exportBoardData(boardId: string): Promise<ExportBoardData | null> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return null
  return fetchExportBoard(supabase, boardId, user.id, 0)
}
